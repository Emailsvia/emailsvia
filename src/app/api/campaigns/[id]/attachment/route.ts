...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Path is prefixed with the user's id so storage RLS ("first folder ==
  // auth.uid()") permits the upload.
  const path = `${u.id}/${id}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${safeName}`;
